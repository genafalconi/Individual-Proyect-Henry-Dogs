    ...
    try {
        ...
        if (name) { // if name exists search in api and db the breed with that name
            let resRazas = await allRazas.filter(elem => elem.name.toLowerCase().includes(name.toLowerCase()));
             ...
             return res.status(200).json(resRazas);
...
router.post('/dog', async (req, res, next) => {
    try {
        ...
        allRazas.forEach(elem => {
            if (elem.name.toLowerCase() === name.toLowerCase()) {
                res.status(404).json({ error: 'The breed already exists' })
            }
        })
        ...
        for (let i = 0; i < temperament.length; i++) {
            try {
                ...
                await newRaza.addTemperamento(idTemp)
            } catch (error) {
                next(error)
            }
        }
...
router.get('/:idRaza', async (req, res, next) => {
    try {
        ...
        if (idRaza) {
            ...
            else return res.status(404).json({ message: 'There is not a breed with that id' });
        } else {
